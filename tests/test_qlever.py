...
[data]
NAME          = first_steps
DESCRIPTION   = kgsteward qlever first_steps test
...
def qlever_workdir( tmp_path_factory ):
    ...
    # The shipped qlever.yaml resolves these from the environment; run_cmd
...
def test_kgsteward_qlever_init_complete_validate( qlever_workdir ):
...
def test_qlever_data_is_served( qlever_workdir ):
    ...
    client = QleverClient(
        qlever_workdir["qleverfile"], qlever_workdir["qleverdir"], echo = False,
    )
