...
#cAbout relationship of dcat:Dataset and void:Dataset 
...
# way out is to erase the graphdb.home directoy
...
parser = argparse.ArgumentParser(
    description = f"Manage a GraphDB repository using HTTP requests. This script rely on three environment variables (GRAPHDB_URL, GRAPHDB_USERNAME, GRAPHDB_PASSWORD) and a YAML config file." 
)
parser.add_argument( 
    'yamlfile', 
    nargs    = 1,
...
parser.add_argument(
    '-F',
    action = 'store_true',
    help   = 'Force rebuild the core of the RDF database. Implies -I -D -L options.'
)
parser.add_argument(
    '-I',
    action = 'store_true',
    help   = 'Create the repository or force recreate the repository, i.e. erase eveything'
)
parser.add_argument(
    '-D',
    action = 'store_true',
    help   = 'Force relaod all RDF data'
)
parser.add_argument(
    '-d',
    help = f"Force reload RDF data from the supplied comma-separated list of graph name (no space in the list). It might be combined with option -C. ",
)
parser.add_argument(
    '-C',
    action = 'store_true',
    help   = f"Load missing/incomplete graphs to complete the repository."
)
parser.add_argument(
    '-L',
    action = 'store_true',
    help   = 'Provide public free-access to the repository, which is strictly read-only.'
)
parser.add_argument(
    '-U',
    action = 'store_true',
...
parser.add_argument(
    '-Q',
    action='store_true',
    help='Clean, test and reload all SPARQL queries. This is a GraphDB specific option. Note that the queries are global to a web site, i.e. they are not attached to a specific repository. It may fails on SPARQL syntax error!'
)

args = parser.parse_args()

# --------------------------------------------------------- #
# Further processiing of command line arguments
# --------------------------------------------------------- #
...
def replace_env_var( txt ) :
    ...
    if m :
        ...
        if val : 
            ...
            sys.exit( "Environment variable not set: " + m.group( 1 ) ) 
    else :
        return txt
