...
class QleverClient( GenericClient ):
    ...
    def __init__( self, qlever_url, qlever_token = None, echo = True ):
        # This constructor works even if the repo does not exist yet
        super().__init__( 
            qlever_url,
            None,
            None
            # qlever_url + "/repositories/" + repository_id + "/statements",
            # qlever_url + "/repositories/" + repository_id + "/rdf-graphs/service"
        )
        self.qlever_url          = qlever_url
        print_task( "contacting server" )
        # FIXME: add some code to ping the server

#        curl -s http://adsicore:7019 -H "Accept: application/sparql-results+json" -H "Content-type: application/sparql-query" --data "SELECT * WHERE { ?s ?p ?o }"
        
    def get_endpoint_update( self ):
        return "" # FIXME: this is a placeholder

    def list_repository( self ):
        r = http_call({
            'method' : 'get',
            'url'    : self.qlever_url + '/rest/repositories',
    ...
    def rewrite_repository( self, graphdb_config_filename ) :
        http_call({
            'method' : 'DELETE',
            'url'    : self.qlever_url + '/rest/repositories/' + self.repository_id,
            'headers' : self.headers
        })
        http_call({
            'method'  : 'POST',
        ...
        http_call({
            'method'  : 'POST',
            'url'     : self.qlever_url + '/rest/autocomplete/enabled',
            'headers' : { **self.headers, "X-GraphDB-Repository": self.repository_id },
            'params'  : { 'enabled' : 'true' }
        })
        http_call({
            'method'  : 'POST',
            'url'     : self.qlever_url + "/rest/security",
            'headers' : { **self.headers, "X-GraphDB-Repository": self.repository_id },
            'json'    : "true"
        })
    ...
    def free_access( self ) :
        # http_call({
        #     'method'  : 'GET',
        #     'url'     : self.qlever_url + "/rest/class-hierarchy?doReload=true&graphURI=",
        #     'headers' : { **self.headers, "X-GraphDB-Repository": self.repository_id },
        # })
        http_call({
            'method'  : 'POST',
            'url'     : self.qlever_url + "/rest/security/free-access",
            'headers' : self.headers,
            "json"    : {
                "enabled" : "true",
...
# curl -s http://adsicore:7019 -H "Accept: application/sparql-results+json" -H "Content-type: application/sparql-query" --data "SELECT * WHERE { ?s ?p ?o }"
    ...
    def graphdb_call( self, request_args, status_code_ok = [ 200 ], echo = True ) :
        request_args['url'] = self.qlever_url + str( request_args['url'] )
    ...
    def rewrite_prefixes( self, echo = True ):
        r = http_call({
            'method'  : 'DELETE',
            'url'     : self.qlever_url + "/repositories/" + self.repository_id + "/namespaces",
            'headers' : self.headers,
        }, [204], echo )
    ...
    def set_prefix( self, short, long, echo = True ):
        r = http_call({
            'method'  : 'PUT',
            'url'     : self.qlever_url + "/repositories/" + self.repository_id + "/namespaces/" + short,
